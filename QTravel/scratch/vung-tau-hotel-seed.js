...
const hotelsData = require('./vung-tau-hotel.json');

async function main() {
  console.log('Starting seed for Vung Tau hotels...');
  
  // Find Vung Tau city
  const vungtau = await prisma.city.findFirst({
    where: {
      cityNameVi: 'Vũng Tàu'
    }
  });
  ...
  if (!vungtau) {
    console.error('City Vũng Tàu not found in database!');
    return;
  }

  console.log(`Found Vung Tau with ID: ${vungtau.id}`);
